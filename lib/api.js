...
export const topicsApi = {
  ...
  startTest: (topicId, lessonId) => post(`/topics/${topicId}/lessons/${lessonId}/start-test`),
  submitTest: (topicId, lessonId, answers) => post(`/topics/${topicId}/lessons/${lessonId}/submit-test`, { answers }),
  // formData: images[] — pazl rasmlari (bittadan yoki bir nechta)
  addPuzzleImages: (topicId, lessonId, formData) => upload(`/topics/${topicId}/lessons/${lessonId}/puzzle-images`, formData),
  removePuzzleImage: (topicId, lessonId, imageId) =>
    del(`/topics/${topicId}/lessons/${lessonId}/puzzle-images/${imageId}`),
};

export const puzzlesApi = {
  lesson: (lessonId) => get(`/puzzles/lesson/${lessonId}`),
  start: (imageId, difficulty) => post(`/puzzles/${imageId}/start`, { difficulty }),
  solve: (imageId, difficulty) => post(`/puzzles/${imageId}/solve`, { difficulty }),
};
...
export const assignmentsApi = {
  ...
  startTest: (id) => post(`/assignments/${id}/start-test`),
...
/**
 * Avtorizatsiyasiz ochiladigan rasm manzili.
 *
 * `fileUrl` avtorizatsiya talab qiladi, shuning uchun mehmon ko'radigan
 * joylarda (bosh sahifadagi banner, ulashilgan post) u 401 qaytaradi va rasm
 * umuman chizilmaydi. Bu manzil esa faqat ochiq mazmunga — banner yoki e'lon
 * qilingan postga biriktirilgan rasmlarga ruxsat beradi.
 */
export function publicFileUrl(fileId) {
  return `${API_URL}/files/public/${fileId}`;
}
